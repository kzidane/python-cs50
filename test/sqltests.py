...
class SQLTests(unittest.TestCase):
    def test_delete_returns_affected_rows(self):
        ...
        self.assertEqual(self.db.execute("DELETE FROM cs50 WHERE id = :a or id = :b",\
            a=rows[1]["id"], b=rows[2]["id"]), 2)
    ...
    def test_select_where(self):
        ...
        self.assertEqual(self.db.execute("SELECT * FROM cs50 WHERE id = :id OR val = :val",\
            id=rows[1]["id"], val=rows[2]["val"]), rows[1:3])
